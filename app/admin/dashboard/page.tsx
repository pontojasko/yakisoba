...
import { Info, LayoutDashboard, TrendingUp, ShoppingBag, Receipt, Users } from "lucide-react";
import {
  BarChart,
  Bar,
  XAxis,
...
export default function DashboardPage() {
  ...
  const statCards = [
    ...
    { label: "Faturamento do Mês", value: formatCurrency(stats.monthRevenue), icon: Receipt, color: "text-primary" },
    {
      ...
      info: "Valor médio gasto por pedido no mês (faturamento do mês ÷ pedidos do mês).",
    },
  ];
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map(({ label, value, icon: Icon, color, info }) => (
                ...
                {info && (
                  <div className="relative group inline-flex">
                    <button
                      ...
                      aria-label="Mais informações sobre ticket médio"
                    >
                      <Info className="h-3.5 w-3.5" />
                    </button>
                    <div
                      role="tooltip"
                      className="pointer-events-none absolute left-1/2 top-full z-20 mt-2 w-64 -translate-x-1/2 rounded-md border bg-popover px-3 py-2 text-xs text-popover-foreground opacity-0 shadow-md transition-opacity group-hover:opacity-100 group-focus-within:opacity-100"
                    >
                      {info}
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-2xl border p-5">
          <h2 className="font-semibold mb-4">Produtos do Mês</h2>
